...
export default function TeacherSubjectsAssignmentList(props) {
        ...
        <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none space-x-2">
          ...
          <div className="inline-flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-offset-2 sm:w-auto">
            <AssignTeacherSubject
              create={props.assignSubject}
              teachers={props && props.teachers}
                  ...
                  {props &&
                    Object.values(props.teachersSubjects).map((data, key) => (
                      <tr key={key}>
                        ...
                        <td className="whitespace-nowrap px-2 py-2 text-sm text-gray-500">
                          {data.map((theForm, kkk) => {
                            return theForm.form == 3 ? (
                              <span key={kkk}>
                                {theForm.Subject==null?"":theForm.Subject.name} <br />
                              </span>
                            ) : (
                              ""
                            );
                        ...
                        <td className="relative whitespace-nowrap py-2 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                          <span className="text-sky-700 hover:text-sky-900">
                            {data[0].AcademicYear.academicYear} - Term{" "}
                            {data[0].AcademicYear.term}
                          </span>
                        </td>
                      </tr>
                    ))}
